...
/// <summary>
/// 自定义层级面板
/// 代码来自@https://www.jianshu.com/p/15a63c338c99
/// </summary>
[InitializeOnLoad]
public static class CustomHierarchy { 
    ...
    public static bool EnableCustomHierarchyLabel = false;
    ...
    private static GUIStyle LabelStyle(Color color) {
        var style = new GUIStyle("AssetLabel");// { padding = { left = EditorStyles.label.padding.left, top = EditorStyles.label.padding.top + 1 }, normal = { textColor = color } };
        return style;
    }
    ...
    /// <param name="index">图标顺序,从右往左</param>
    /// <returns></returns>
    ...
    // 综合以上，根据类型，绘制图标和文字 
    private static void DrawRectIcon<T>(Rect selectionRect, GameObject go, Color textColor, ref int order, ref GUIStyle style) where T : Component { //if (go.GetComponent<T>()) 
        // 使用扩展方法HasComponent 
