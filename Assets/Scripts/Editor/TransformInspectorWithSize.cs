...
public class TransformInspectorWithSize : UnityEditor.Editor
{

    private const float FIELD_WIDTH = 212.0f * 2;
    ...
    {
        EditorGUIUtility.wideMode = TransformInspectorWithSize.WIDE_MODE;
        EditorGUIUtility.labelWidth = EditorGUIUtility.currentViewWidth - TransformInspectorWithSize.FIELD_WIDTH; // align field to right of inspector
    ...
    {
        if (Mathf.Abs(position.x) > TransformInspectorWithSize.POSITION_MAX) return false;
        if (Mathf.Abs(position.y) > TransformInspectorWithSize.POSITION_MAX) return false;
        if (Mathf.Abs(position.z) > TransformInspectorWithSize.POSITION_MAX) return false;
        return true;
    }
